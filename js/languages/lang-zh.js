...
LANGUAGES.ZH = {
    ...
    text: {
        ...
        'research-p2-name': "免费的鲨鱼升级",
        'research-p2-desc': `用${toTextStyle("鱼",'fish')}购买的${toTextStyle("鲨鱼",'shark')}升级不再消耗${toTextStyle("鱼",'fish')}。`,

        'research-p3-name': "强化鲨鱼等级",
        'research-p3-desc': `${toTextStyle("鲨鱼",'shark')}等级的价格基数减 <b>1</b>, 每次购买后强制进行        ${toTextStyle("重生",'prestige')}，重置你的${toTextStyle("重生",'prestige')}碎片和${toTextStyle("重生",'prestige')}升级。解锁新的${toTextStyle("鲨鱼",'shark')}升级。`,
        ...
        get 'reset-core-message'() {
            ...
            return `
            <h3>${c}</h3><br>            ${c}是第二层重置。进入地核除了重置${p}所重置的资源，还重置${p}碎片、${p}升级、几乎所有的${toTextStyle('研究','prestige')}和探索进度，获得${m}碎片.
            第一次进入地核将解锁${c}反应堆。<br>
        ...
        'radio-desc' : {
            ...
            'comma-format': ['最大逗号数量',['3','6','9','12','15']],
            'autosave': ['自动存档',['禁用','启用']],
        ...
        'option-buttons-text': ["存档","导出到剪贴板","导出到文件","导入存档文本","导入存档文件","删档！！！"],
